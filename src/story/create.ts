...
export const storyCreate = functions.firestore
  .document(`${shared.MY_STORIES}/{id}`)
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      if (storyFollowers.length > 0) {
        ...
        const batches = _.chunk(storyFollowers, shared.MAX_BATCH_SIZE).map(
          userIds => {
            const writeBatch = db.batch();
            userIds.forEach(userId => {
              const id =
                userId + '_' + storyid + '_' + new Date().getTime().toString();
              writeBatch.set(
                db
                  .collection('users')
                  .doc(userId)
        ...
        console.log(
          'The feed of story followers# ',
          storyFollowers.length,
          ' have been update'
        );
      }
    } catch (err) {
...
async function getUserFollowersIds(userid: string): Promise<string[]> {
  const forumGroupFollwersRef = db
    .collection(shared.FOLLOWER_STORY)
